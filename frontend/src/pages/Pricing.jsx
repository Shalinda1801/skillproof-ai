...
const plans = [
  {
    ...
    price: "Free",
    subtitle:
      "For learners starting skill verification",
    icon: Sparkles,
...

  {
    ...
    price: "LKR 1,500",
    subtitle:
      "For verified certificates and public proof",
    icon: Award,
...

  {
    ...
    price: "Custom",
    subtitle:
      "For training centers and academic teams",
    icon: Users,
        ...
        <section className="mx-auto max-w-7xl">
          <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <div>
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">
                Start free with skill challenges,
                then upgrade when you need a
                verifier-approved certificate
                with public QR verification.
              </p>
            ...
            <div className="premium-card pro-card rounded-[2.5rem] p-8">
              <div className="grid h-16 w-16 place-items-center rounded-3xl border border-indigo-100 bg-indigo-50 text-indigo-600">
                <CreditCard size={34} />
              </div>
              ...
              <p className="mt-4 leading-8 text-slate-600">
                SkillProof uses a product-style
                checkout flow for paid certificate
                access and public proof.
              </p>

              <div className="mt-6 rounded-3xl border border-slate-200 bg-white/85 p-5 shadow-sm">
          ...
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {plans.map((plan) => (
              <div
                key={plan.name}
                className={`premium-card pro-card relative rounded-[2rem] p-7 ${
                  plan.highlighted
                    ? "border-indigo-300 shadow-[0_24px_70px_rgba(79,70,229,0.16)]"
                    : ""
                }`}
              >
                {plan.highlighted && (
                  <div className="mb-5 inline-flex rounded-full border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-black text-indigo-700">
                    Recommended
                  </div>
                )}

                <div className="grid h-14 w-14 place-items-center rounded-2xl border border-slate-200 bg-white text-indigo-600 shadow-sm">
                  <plan.icon size={28} />
                </div>
          ...
          <section className="mt-14 grid gap-6 lg:grid-cols-3">
            {trustPoints.map((item) => (
              <div
                key={item.title}
                className="premium-card pro-card rounded-[2rem] p-6"
              >
                <item.icon
