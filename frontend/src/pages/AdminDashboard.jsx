...
const AdminDashboard = () => {
  ...
  const [
    personalizedChallengeRequests,
    setPersonalizedChallengeRequests,
  ] = useState([]);
  ...
  const getAssessmentFromResponse = (
    response
  ) =>
    response?.assessment ||
  ...
  const loadAdminData = async ({
    ...
    try {
      ...
      const loadedChallengeRequests =
        challengeRequestData.requests || [];
      ...
      setSelectedSubmission(
        (currentSubmission) => {
          ...
          if (
            updatedSelectedSubmission.status ===
            "REJECTED"
          ) {
            return null;
          }
  ...
  const handleRunAi = async () => {
    ...
    try {
      ...
      const response =
        await adminApi.runAiAssessment(
          selectedSubmission._id
        );

      const assessment =
        getAssessmentFromResponse(response);
    ...
    try {
      ...
      await adminApi.updateSubmissionStatus(
        selectedSubmission._id,
        {
          ...
          reviewNote:
            reviewNote ||
            "Submission rejected after review.",
        }
      );
  ...
  const handleApproveAiChallengeRequest =
    async (requestId) => {
      try {
        setActionLoading(
          `approve-ai-${requestId}`
        );

        setMessage("");
        setError("");

        await challengeApi.approvePersonalizedChallengeRequest(
          requestId,
          {
            adminNote:
              "AI-generated personalized challenge approved by admin.",
          }
        );

        setMessage(
          "AI challenge request approved successfully."
        );
  ...
  const handleRejectAiChallengeRequest =
    async (requestId) => {
      try {
        setActionLoading(
          `reject-ai-${requestId}`
        ...
        await challengeApi.rejectPersonalizedChallengeRequest(
          requestId,
          {
            adminNote:
              "AI-generated personalized challenge rejected by admin.",
          }
        );

        setMessage(
          "AI challenge request rejected."
        );
  ...
  const pendingAiChallengeRequests =
    personalizedChallengeRequests.filter(
      (request) =>
        request.status === "PENDING"
    );

  const pendingAiChallengeCount =
    pendingAiChallengeRequests.length;
      ...
      <main className="relative z-10 min-h-screen px-6 py-6 text-slate-900">
        <div className="mx-auto max-w-7xl">
          ...
          <section className="glass-card pro-card rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-[0_20px_65px_rgba(79,70,229,0.10)] backdrop-blur-xl">
            <p className="text-sm font-bold uppercase tracking-[0.3em] text-violet-600">
              Verifier workspace
            </p>

            <h1 className="mt-3 max-w-5xl text-4xl font-black leading-tight md:text-5xl">
              Welcome {user?.name}, review
              submissions and issue trusted
              certificates.
            </h1>

            <p className="mt-4 max-w-3xl text-slate-600">
              Manage student evidence, run
              AI-assisted assessment, approve
              submissions, and generate
          ...
          {loading ? (
            ...
            <>
              ...
              <section className="glass-card mt-8 rounded-[2rem] border border-slate-200 bg-white/85 p-6 shadow-[0_18px_55px_rgba(51,65,85,0.08)]">
                <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-black">
                      Personalized AI Challenge
                      Requests
                    </h2>

                    <p className="mt-2 text-sm text-slate-600">
                      Open this panel only when
                      you want to review generated
                      challenge drafts.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-3 text-sm">
                      <span className="text-slate-600">
                        Pending:
                      </span>{" "}
                ...
                {showAiChallengeReview && (
                  <>
                    ...
                    <div className="mt-6 grid gap-5 lg:grid-cols-2">
                      {pendingAiChallengeRequests.map(
                        (request) => (
                          <div
                            key={request._id}
                          ...
                          >
                            ...
                            <div className="mt-5 max-h-48 overflow-y-auto rounded-2xl border border-slate-200 bg-white/90 p-4">
                              <h4 className="font-black text-indigo-600">
                            ...
                            <div className="mt-4 grid gap-4 md:grid-cols-2">
                              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                <h4 className="font-black text-violet-600">
                                  Challenge Details
                                </h4>
                              ...
                              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                ...
                                <ul className="mt-3 space-y-2 text-sm text-slate-700">
                                  {(
                                    request.generatedRequiredEvidence ||
                                    []
                                  ).map(
                                    (
                                      item,
                            ...
                            <div className="mt-5 grid gap-3 md:grid-cols-2">
                              <button
                                onClick={() =>
                                  handleApproveAiChallengeRequest(
                                    request._id
                                  )
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleRejectAiChallengeRequest(
                                    request._id
                                  )
                                }
                    ...
                    {pendingAiChallengeRequests.length ===
                      0 && (
                      <p className="mt-6 rounded-2xl bg-slate-50 p-4 text-slate-600">
                        No pending AI challenge
                        requests. Approved and
                        rejected requests are
                        hidden from this review
                        panel.
                      </p>
                    )}
                  </>
                ...
                <div className="glass-card rounded-[2rem] border border-slate-200 bg-white/85 p-6 shadow-[0_18px_55px_rgba(51,65,85,0.08)] backdrop-blur-xl">
                  ...
                  {shouldShowReviewWorkspace ? (
                    <>
                      ...
                      {aiAssessmentResult && (
                        <div className="mt-5 rounded-3xl border border-violet-300 bg-violet-50 p-5">
                          ...
                          <div className="mt-5 grid gap-4 md:grid-cols-3">
                            <div className="rounded-2xl border border-slate-200 bg-white p-4">
                              <h5 className="font-black text-emerald-600">
                                Strengths
                              </h5>

                              <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700">
                                {(
                                  aiAssessmentResult.strengths ||
                                  []
                                ).map(
                                  (
                                    item,
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-4">
                              <h5 className="font-black text-amber-600">
                                Weaknesses
                              </h5>

                              <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700">
                                {(
                                  aiAssessmentResult.weaknesses ||
                                  []
                                ).map(
                                  (
                                    item,
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-white p-4">
                              <h5 className="font-black text-indigo-600">
                                Improvements
                              </h5>

                              <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700">
                                {(
                                  aiAssessmentResult.improvements ||
                                  []
                                ).map(
                                  (
                                    item,
                          ...
                          <div className="mt-4 flex flex-wrap gap-3 text-xs text-slate-600">
                            <span className="rounded-full bg-slate-100 px-3 py-1">
                              Provider:{" "}
                              {aiAssessmentResult.provider ||
                                "MOCK/GEMINI"}
                            </span>

                            <span className="rounded-full bg-slate-100 px-3 py-1">
                              Model:{" "}
                              {aiAssessmentResult.model ||
                                "N/A"}
                            </span>
                          </div>
                        </div>
